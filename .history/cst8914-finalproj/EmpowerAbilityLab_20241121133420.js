...
function openModal(event) {
    ...
    // Collect all focusable elements except the close button
    focusableElements = Array.from(
        modal.querySelectorAll(
            'button, a, input, textarea, select, [tabindex]:not([tabindex="-1"])'
        )
    ).filter((el) => !el.classList.contains('close-button'));
    ...
    // Focus the modal dialog itself or the first focusable element
