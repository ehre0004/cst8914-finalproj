...
function openModal(event) {
    ...
    // Note: Removed tabindex="-1" and aria-label
    ...
    modalTitle.id = 'modalTitle'; // Associate with aria-labelledby
    modalTitle.textContent = 'Meet the Empower Community';
    // Removed aria-hidden="true" so the heading is read as part of the modal label
    ...
    // Exclude the close button from initial focus if desired
    const initialFocusableElements = focusableElements.filter(
    ...
    if (initialFocusableElements.length > 0) {
        ...
        // Fallback to focusing the close button or any other focusable element
        focusableElements[0].focus();
    } else {
        // If no focusable elements, set focus on the modal container
