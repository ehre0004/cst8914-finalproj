...
let isModalOpening = false; // New flag to control focus management during modal opening
...
function openModal(event) {
    ...
    // Create a visually hidden element for the modal label
    const modalLabel = document.createElement('div');
    modalLabel.id = 'modalLabel';
    modalLabel.className = 'visually-hidden';
    modalLabel.textContent = 'Meet the Empower Community dialog';
    ...
    // Focus the modal dialog container itself
    modal.focus();

    // Add focus trap event listeners after focus is set
...
function trapTabKey(event) {
    if (!modalElement || focusableElements.length === 0) return;
...
function maintainFocus(event) {
    if (isModalOpening) return; // Do not refocus during modal opening
...
// CSS to visually hide the modal label
